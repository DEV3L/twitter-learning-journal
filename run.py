...
manual_keyword_detail_classification = {
    ...
    # pages = 1364
    # wpp = 200
    # pages * wpp = total_words
    # total_days = (book.stop_date - book.start_date) + 1
    # total_days = 15
    # total_words =
...
if __name__ == '__main__':
    ...
    books = []
    books.extend(get_books())

    tweets = tweet_dao.query_all()
    _tweets = [tweet for tweet in tweets if tweet.created_at >= min_date and tweet.created_at <= max_date]

    details = tweet_dao._database.query(Detail).all()
    _details = [detail for detail in details if detail.start_date >= min_date and detail.stop_date <= max_date]

    # load
    # timeline(min_date, max_date, _tweets, _details, audio_books, books)
